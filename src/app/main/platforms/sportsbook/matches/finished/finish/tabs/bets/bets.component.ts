...
export class BetsComponent extends BasePaginatedGridComponent implements OnInit {
  ...
  constructor(
    ...
    this.detailCellRendererParams = {
      ...
      detailGridOptions: {
        getContextMenuItems:
          this.getContextMenuItems.bind(this),
        rowHeight: 47,
        defaultColDef: {
          sortable: true,
          filter: true,
        ...
        if (params) {
          this.apiService.apiPost('report/selections', { BetId: params.data.Id })
            ...
            .subscribe((data) => {
              ...
              nestedRowData.forEach(element => {
                element.SelectionStatus = BetStatuses[element.SelectionStatus]
              });
  ...
  createServerSideDatasource() {
    return {
      getRows: (params) => {
        ...
        this.setFilterDropdown(params, filterNames);
          ...
          .subscribe(data => {
            if (data.Code === 0) {
              const mappedRows = data.Objects.map((items) => {
                items['State'] = BetStatuses[items['State']]
