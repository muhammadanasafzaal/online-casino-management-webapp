...
export class TranslationsComponent extends BasePaginatedGridComponent implements OnInit {
  ...
  createServerSideDatasource = () => {
    return {
      getRows: (params) => {
        this.filter.PageIndex = params.request.startRow / 100;
  ...
  //   this.filter.ObjectTypeId = this.objectTypeId;
  //   this.filter.SelectedLanguages = this.languageModel.map(el => {
  //     return el.Id
  //   });

  //   this.apiService.apiPost('common/translationentries', this.filter)
  //     .pipe(take(1))
  //     .subscribe(data => {
  //       if (data.ResponseCode === 0) {
  //         this.translationEntries = data.ResponseObject.Entities;
  //         this.length = data.ResponseObject.TotalCount;
