...
export class UserSettingsComponent implements OnInit {
  ...
  private mergeUserSettingsApi() {
    ...
    this.apiService.apiPost(this.configService.getApiUrl, requestObject, true, Controllers.ENUMERATION, Methods.GET_AGENT_LEVELS_ENUM)
      .pipe(
        mergeMap(data => {
          this.agentsLevelsEnums = data.ResponseObject;
          this.setAgentLevelsEnumResponse(data);
          return this.apiService.apiPost(this.configService.getApiUrl, requestObject, true, Controllers.USER, Methods.GET_USER_SETTINGS);
        }))
      .subscribe(data => {
